...
본 프로젝트는 **OTA(Over-The-Air) 소프트웨어 업데이트 수행 중 차량 원격 제어 기능을 제한하는 안전 제어 로직**을 구현하는 Java 기반 Toy 프로젝트입니다.

OTA 설치 및 재부팅 구간에서 원격 제어 요청이 차량 시스템 상태와 충돌할 수 있으므로, 해당 구간에서 차량 상태 변경 기능을 제한하고 OTA 완료 후 정상 기능을 복구하는 것을 목표로 합니다.

본 프로젝트는 **자동차 Connected Car Service(CCS) 환경을 가정한 소프트웨어 구조**를 기반으로 설계되었습니다.
...
본 문서는 OTA 업데이트 진행 중 차량 원격 제어 기능을 안전하게 제한하기 위한 소프트웨어 요구사항을 정의합니다.

OTA 설치 및 시스템 재부팅 구간에서 원격 명령이 실행되는 것을 방지하고, 업데이트 완료 또는 실패 시 원격 기능을 정상적으로 복구하는 것을 목적으로 합니다.
...
## 13.1 1차 구현 범위 (Minimum Viable Product)

초기 구현에서는 다음 기능을 포함한다.
